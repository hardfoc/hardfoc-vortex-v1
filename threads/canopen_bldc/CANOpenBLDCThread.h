...
public:
  ...
  bool SetPositionMode(int32_t position, uint32_t velocity = 0,
  ...
  bool SetProfileParameters(uint32_t velocity, uint32_t acceleration,
...
private:
  ...
  bool waitForStateTransition(DriveState targetState,
                              uint32_t timeoutMs = 5000);

  // SDO communication helpers
  bool sendSDODownload(uint16_t index, uint8_t subIndex, uint32_t data,
                       uint8_t size);
  ...
  CanFrame buildSDOFrame(uint8_t command, uint16_t index, uint8_t subIndex,
                         uint32_t data = 0);
  ...
  static constexpr uint32_t STATUS_REQUEST_INTERVAL_MS =
      100; ///< Status request interval
  static constexpr uint32_t COMMUNICATION_TIMEOUT_MS =
      5000; ///< Communication timeout
