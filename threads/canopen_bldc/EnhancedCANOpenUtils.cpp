...
CanFrame EnhancedCANOpenUtils::BuildEmergency(uint8_t nodeId, uint16_t errorCode, 
                                            uint8_t errorRegister, uint32_t manufacturerData) noexcept {
    CanFrame frame{};
...
CanFrame EnhancedCANOpenUtils::BuildPDO(uint8_t nodeId, uint8_t pdoNumber, bool isReceive,
    ...
    if (isReceive) {
        // RPDO (Receive PDO)
        switch (pdoNumber) {
            case 1: frame.id = 0x200 + nodeId; break;
            case 2: frame.id = 0x300 + nodeId; break;
            case 3: frame.id = 0x400 + nodeId; break;
            case 4: frame.id = 0x500 + nodeId; break;
            default: frame.id = 0x200 + nodeId; break;
        }
    } else {
        // TPDO (Transmit PDO)
        switch (pdoNumber) {
            case 1: frame.id = 0x180 + nodeId; break;
            case 2: frame.id = 0x280 + nodeId; break;
            case 3: frame.id = 0x380 + nodeId; break;
            case 4: frame.id = 0x480 + nodeId; break;
            default: frame.id = 0x180 + nodeId; break;
        }
    }
...
CanFrame EnhancedCANOpenUtils::BuildTime(uint32_t timeOfDay, uint16_t days) noexcept {
    CanFrame frame{};
...
CanFrame EnhancedCANOpenUtils::BuildManufacturerSpecific(uint8_t nodeId, uint8_t functionCode,
                                                        const uint8_t* data, uint8_t dataLength) noexcept {
    CanFrame frame{};
...
uint8_t EnhancedCANOpenUtils::BuildMotorEnableSequence(uint8_t nodeId, CanFrame* frames, uint8_t maxFrames) noexcept {
    ...
    // 1. Shutdown
    frames[frameCount++] = BuildControlWord(nodeId, static_cast<uint16_t>(ControlWordCommand::Shutdown));

    // 2. Switch On
    frames[frameCount++] = BuildControlWord(nodeId, static_cast<uint16_t>(ControlWordCommand::SwitchOn));

    // 3. Enable Operation
    frames[frameCount++] = BuildControlWord(nodeId, static_cast<uint16_t>(ControlWordCommand::EnableOperation));

    return frameCount;
}

uint8_t EnhancedCANOpenUtils::BuildMotorDisableSequence(uint8_t nodeId, CanFrame* frames, uint8_t maxFrames) noexcept {
    ...
    // Disable voltage
    frames[frameCount++] = BuildControlWord(nodeId, static_cast<uint16_t>(ControlWordCommand::DisableVoltage));

    return frameCount;
}

CanFrame EnhancedCANOpenUtils::BuildInterpolationPeriod(uint8_t nodeId, uint8_t subIndex, uint8_t value) noexcept {
    return BuildSdoDownload(nodeId, 0x60C0, subIndex, value, 1);
}

uint8_t EnhancedCANOpenUtils::BuildBLDCConfigSequence(uint8_t nodeId, CanFrame* frames, uint8_t maxFrames,
                                                     uint8_t polePairs, uint32_t encoderResolution) noexcept {
    ...
    frames[frameCount++] = BuildSdoDownload(nodeId, ObjectDictionary::MOTOR_TYPE, 0x00, 3, 2);  // BLDC motor

    // 2. Set encoder resolution
    frames[frameCount++] = BuildSdoDownload(nodeId, ObjectDictionary::ENCODER_INCREMENTS, 0x00, encoderResolution, 4);

    // 3. Set profile acceleration (default value)
    frames[frameCount++] = BuildSdoDownload(nodeId, ObjectDictionary::PROFILE_ACCELERATION, 0x00, 10000, 4);

    // 4. Set profile deceleration (default value)
    frames[frameCount++] = BuildSdoDownload(nodeId, ObjectDictionary::PROFILE_DECELERATION, 0x00, 10000, 4);
...
bool EnhancedCANOpenUtils::ExtractSDOIndex(const CanFrame& frame, uint16_t& index, uint8_t& subIndex) noexcept {
...
EnhancedCANOpenUtils::DriveState EnhancedCANOpenUtils::DecodeStatusWord(uint16_t statusWord) noexcept {
