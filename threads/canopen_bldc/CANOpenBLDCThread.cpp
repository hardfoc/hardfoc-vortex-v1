...
CANOpenBLDCThread::CANOpenBLDCThread(uint8_t nodeId, uint32_t canBaudRate)
    : BaseThread("CANOpenBLDC"), m_canInterface(nullptr), m_config{},
      m_motorStatus{}, m_canBaudRate(canBaudRate), m_lastHeartbeat(0),
      m_lastStatusRequest(0), m_lastCommunication(0),
      m_currentState(DriveState::NotReadyToSwitchOn),
      m_operationMode(OperationMode::PositionMode), m_initialized(false),
...
bool CANOpenBLDCThread::Setup() {
  ...
  m_canInterface =
      std::make_unique<FlexCan>(0, m_canBaudRate, TWAI_TX_PIN, TWAI_RX_PIN);
...
CANOpenBLDCThread::DriveState
CANOpenBLDCThread::decodeStatusWord(uint16_t statusWord) {
...
bool CANOpenBLDCThread::EnableMotor() {
  ...
  default:
    console_warn(TAG, "Cannot enable motor in current state: %d",
...
bool CANOpenBLDCThread::SetPositionMode(int32_t position, uint32_t velocity,
  ...
  console_info(TAG, "SetPositionMode: pos=%ld, vel=%lu, acc=%lu", position,
...
bool CANOpenBLDCThread::SetVelocityMode(int32_t velocity,
  ...
  console_info(TAG, "SetVelocityMode: vel=%ld, acc=%lu", velocity,
               acceleration);
  return true;
}
