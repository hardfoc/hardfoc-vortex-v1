...
const WS2812Animator::Effect WS2812TestThread::s_effectList[] = {
    WS2812Animator::Effect::SolidColor, WS2812Animator::Effect::Rainbow,
    WS2812Animator::Effect::Breath,     WS2812Animator::Effect::Chase,
    WS2812Animator::Effect::Blink,      WS2812Animator::Effect::Larson};

const size_t WS2812TestThread::s_effectCount =
    sizeof(s_effectList) / sizeof(s_effectList[0]);

WS2812TestThread::WS2812TestThread(uint8_t ledCount, uint32_t updateIntervalMs)
    : BaseThread("WS2812Test"), m_strip(nullptr), m_animator(nullptr),
      m_ledCount(ledCount), m_updateIntervalMs(updateIntervalMs),
...
void WS2812TestThread::runCurrentEffect() {
  ...
  case WS2812Animator::Effect::SolidColor: {
    ...
    uint32_t colorIndex =
        (elapsed / 1000) % (sizeof(colors) / sizeof(colors[0]));
    m_animator->setEffect(WS2812Animator::Effect::SolidColor,
                          colors[colorIndex]);
  } break;
  ...
  case WS2812Animator::Effect::Breath:
    m_animator->setEffect(WS2812Animator::Effect::Breath,
                          0x0000FF); // Blue breathing
    break;
  ...
  case WS2812Animator::Effect::Blink:
    m_animator->setEffect(WS2812Animator::Effect::Blink,
                          0xFFFFFF); // White blink
    break;
  case WS2812Animator::Effect::Larson:
    m_animator->setEffect(WS2812Animator::Effect::Larson,
                          0xFF0000); // Red larson scanner
    break;
  default:
    m_animator->setEffect(WS2812Animator::Effect::SolidColor,
                          0x404040); // Dim white fallback
    break;
  }
